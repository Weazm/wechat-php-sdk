...
class Wechat {
    ...
    const GROUP_MEMBER_URL = '/groups/getid?'; //mod
    const GROUP_CREATE_URL = '/groups/create?';
    const GROUP_UPDATE_URL = '/groups/update?';
    const GROUP_MEMBER_UPDATE_URL = '/groups/members/update?';
    const CUSTOM_SEND_URL = '/message/custom/send?';
    const GROUP_SEND_URL = '/message/mass/sendall?';
    const UPLOAD_GROUP_MSG_NEWS = '/media/uploadnews?';
    ...
    public function valid($return = false) {
        ...
        if ($return) {
            ...
            } else return $this->checkSignature();
        } else {
            if ($echoStr) {
    ...
    public function getRevFrom() {
        if (isset($this->_receive['FromUserName'])) return $this->_receive['FromUserName'];
        else return false;
    }
    /**
     * 获取消息接受者
    ...
    public function getRevTo() {
        if (isset($this->_receive['ToUserName'])) return $this->_receive['ToUserName'];
        else return false;
    }
    /**
     * 获取接收消息的类型
    ...
    public function getRevType() {
        if (isset($this->_receive['MsgType'])) return $this->_receive['MsgType'];
        else return false;
    }
    /**
     * 获取消息ID
    ...
    public function getRevID() {
        if (isset($this->_receive['MsgId'])) return $this->_receive['MsgId'];
        else return false;
    }
    /**
     * 获取消息发送时间
    ...
    public function getRevCtime() {
        if (isset($this->_receive['CreateTime'])) return $this->_receive['CreateTime'];
        else return false;
    }
    /**
     * 获取接收消息内容正文
    ...
    public function getRevContent() {
        if (isset($this->_receive['Content'])) return $this->_receive['Content'];
    ...
    public function getRevPic() {
        if (isset($this->_receive['PicUrl'])) return $this->_receive['PicUrl'];
        else return false;
    }
    /**
     * 获取接收消息链接
    ...
    public function getRevEvent() {
        if (isset($this->_receive['Event'])) {
            $return = array('event' => $this->_receive['Event']);
            if (isset($this->_receive['EventKey'])) {
                $return['eventkey'] = $this->_receive['EventKey'];
            }
            if (isset($this->_receive['Ticket'])) {
                $return['ticket'] = $this->_receive['Ticket'];
            }
            if (isset($this->_receive['Latitude'])) {
                $return['location'] = array("latitude" => $this->_receive['Latitude'], "longitude" => $this->_receive['Longitude'], "precision" => $this->_receive['Precision']);
            }
            return $return;
        } else return false;
    }
    ...
    public function image($media_id = '') {
        ...
        $msg = array('ToUserName' => $this->getRevFrom(), 'FromUserName' => $this->getRevTo(), 'MsgType' => self::MSGTYPE_IMAGE, 'Image' => array("MediaId" => $media_id), 'CreateTime' => time(), 'FuncFlag' => $FuncFlag);
        $this->Message($msg);
        return $this;
    }
    /**
     * 设置回复消息
     * Examle: $obj->image()->reply();
     * @param string $text
     */
    public function text($text = '') {
        $FuncFlag = $this->_funcflag ? 1 : 0;
        $msg = array('ToUserName' => $this->getRevFrom(), 'FromUserName' => $this->getRevTo(), 'MsgType' => self::MSGTYPE_TEXT, 'Content' => $text, 'CreateTime' => time(), 'FuncFlag' => $FuncFlag);
        $this->Message($msg);
        return $this;
    }
    /**
    ...
    public function checkAuth($appid = '', $appsecret = '') {
        ...
        if ($result) {
            ...
            if (!$json || isset($json['errcode'])) {
                ...
                return array('code' => $json['errcode'], 'msg' => $json['errmsg']);
            }
    ...
    public function getMedia($media_id) {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_get(self::API_FILE_URL_PREFIX . self::MEDIA_GET_URL . 'access_token=' . $this->access_token . '&media_id=' . $media_id);
        if ($result) {
            $json = json_decode($result, true);
    ...
    private function http_file_post($url, $param) {
        $oCurl = curl_init();
        curl_setopt($oCurl, CURLOPT_URL, $url);
        curl_setopt($oCurl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($oCurl, CURLOPT_POST, true);
    ...
    public function uploadMedia($file_path, $type) {
        ...
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_file_post(self::API_FILE_URL_PREFIX . self::MEDIA_UPLOAD_URL . 'access_token=' . $this->access_token . '&type=' . $type, $data);
        // $result = $this->http_post(URL::to('/'), $data);
        if ($result) {
            $json = json_decode($result, true);
            if (isset($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    public function getQRCode($scene_id, $type = 0, $expire = 1800) {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array('action_name' => $type ? "QR_LIMIT_SCENE" : "QR_SCENE", 'expire_seconds' => $expire, 'action_info' => array('scene' => array('scene_id' => $scene_id)));
        if ($type == 1) {
     ...
     * @return boolean|array
     */
    public function getGroupMember($openid) {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $data = array('openid' => $openid);
        $result = $this->http_post(self::API_URL_PREFIX . self::GROUP_MEMBER_URL . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
    ...
    */
    ...
    public function getSignature($arrdata, $method = "sha1") {
        ...
        foreach ($arrdata as $key => $value) {
            if (strlen($paramstring) == 0) $paramstring.= $key . "=" . $value;
    ...
    */
    ...
    /**
     * [sendGroupMessage description]
     * @param  [type]  $data        [description]
     * @param  boolean $openid_list [description]
     * @param  boolean $group_id    [description]
     * @return [type]               [description]
     */
    public function sendGroupMessage($data, $openid_list = false, $group_id = false) {
        // UPLOAD_GROUP_MSG_NEWS
        if (!$this->access_token && !$this->checkAuth()) return false;
        $uploadResult = $this->http_post(self::API_URL_PREFIX . self::UPLOAD_GROUP_MSG_NEWS . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($uploadResult) {
            $json = json_decode($uploadResult, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
                $this->errMsg = $json['errmsg'];
                return array('code' => $json['errcode'], 'msg' => $json['errmsg']);
            ...
            if ($group_id) {
                $sendData = array("filter" => array("group_id" => $group_id), "mpnews" => array("media_id" => $json['media_id']), "msgtype" => "mpnews");
            } else if ($openid_list) {
                $sendData = array("touser" => $openid_list, "mpnews" => array("media_id" => $json['media_id']), "msgtype" => "mpnews");
            }
            $result = $this->http_post(self::API_URL_PREFIX . self::GROUP_SEND_URL . 'access_token=' . $this->access_token, self::json_encode($sendData));
            
            if ($result) {
                $callback = json_encode($result, true);
                if (!$callback || !empty($callback['errcode'])) {
                    $this->errCode = $callback['errcode'];
                    $this->errMsg = $callback['errmsg'];
                    return array('code' => $callback['errcode'], 'msg' => $callback['errmsg']);
                }
                return $callback;
            }
            return $json;
